...
import { Smile, Reply, Edit2, Trash2, Pin, Check, X, Download, File, FileText, Film, Music, FileArchive } from 'lucide-react';
...
export const MessageItem: React.FC<MessageItemProps> = ({
  ...
  return (
    ...
    >
        ...
        <div className="absolute -top-3.5 right-4 z-30 flex items-center bg-[#313338] border border-[#232428] rounded-md shadow-lg p-0.5 space-x-0.5 animate-in fade-in zoom-in-95 duration-100">
          ...
          {isAuthor && (
            <button
              ...
              title="Delete"
              className="p-1.5 hover:bg-[#35373c] text-gray-400 hover:text-[#f23f43] rounded transition-colors"
            >
              <Trash2 size={16} />
            </button>
          )}
      ...
      {showEmojiPicker && (
        <div ref={emojiPickerRef} className="absolute right-4 top-4 z-50">
          <EmojiPicker
            onSelectEmoji={(emoji) => {
      ...
      {/* User Avatar */}
      <div
        className="mr-3.5 flex-shrink-0 pt-0.5"
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-baseline space-x-2">
          <span
            onClick={() => author && onViewProfile && onViewProfile(author)}
            className="font-bold text-xs text-slate-100 hover:text-indigo-400 hover:underline cursor-pointer transition-colors"
          >
        ...
        {message.attachments && message.attachments.length > 0 && (
          <div className="mt-2.5 space-y-2 max-w-lg">
            {message.attachments.map((att) => {
              ...
              const isAudio = att.contentType?.startsWith('audio/') || ['mp3', 'wav', 'ogg', 'm4a'].includes(ext);
              ...
              if (isAudio) {
                return (
                  <div key={att.id} className="p-3.5 rounded-2xl bg-[#0c0e14] border border-white/10 shadow-md max-w-md space-y-2">
                    <div className="flex items-center space-x-2 text-xs font-semibold text-slate-200">
                      <Music size={16} className="text-emerald-400 flex-shrink-0" />
                      <span className="truncate">{att.filename}</span>
                      <span className="text-[10px] text-slate-400 font-mono">({formatSize(att.size)})</span>
                    </div>
